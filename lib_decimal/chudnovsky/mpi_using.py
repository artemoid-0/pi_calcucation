...
def pi_Chudnovsky_MPI_decimal_with_factorial(precision):
    print("Calculating with Chudnovsky formula (decimal) (factorial) in MPI mode")
    ...
    for k in range(rank, precision, size):
        sum_value += get_Chudnovsky_term_dec(k)
...
def pi_Chudnovsky_MPI_decimal_dynamic(precision):
    print("Calculating with Chudnovsky formula (decimal) in MPI mode (dynamic)")
    ...
    if rank == 0:
        ...
        while True:
            ...
            if task_index is None:
                break
            term = get_Chudnovsky_term_dec(task_index)
